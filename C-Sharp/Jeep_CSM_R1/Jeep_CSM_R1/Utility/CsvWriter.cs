...
namespace Utility
{
    ...
    public sealed class CsvWriter : IDisposable
    {
        ...
        /// <summary>
        /// Writes csv content to a file in the encoding specified.  Use null for default.
        /// If bAppend = true, appends the data to the file if it exists.  If false
        /// then deletes the file if it exists.
        /// </summary>
        /// <param name="csvFile">CsvFile</param>
        /// <param name="filePath">File path</param>
        /// <param name="encoding">Encoding</param>
        /// <param name="bAppend">Append Data</param>
        public void WriteCsv(CsvFile csvFile, string filePath, Encoding encoding,bool bAppend)
        {
            if (!bAppend)
            {
                if (File.Exists(filePath))
            ...
            using (StreamWriter writer = new StreamWriter(filePath, bAppend, encoding ?? Encoding.Default))
            {
                WriteToStream(csvFile, writer);
                writer.Flush();
                writer.Close();
